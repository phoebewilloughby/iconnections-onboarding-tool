...
describe('CsmRouter.assignCsm', () => {
  ...
  test('reasoning string is returned', () => {
    const router = new CsmRouter();
    const { reasoning } = router.assignCsm(DEALS[0], CSMS);
    expect(reasoning).toContain('Enterprise');
    expect(reasoning).toContain('Americas');
  });
  ...
  test('round-robin cycles through multiple CSMs in same region pool', () => {
    ...
    const router = new CsmRouter();
    const { csm: a1 } = router.assignCsm(makeDeal({ invoiceAmount: 5_000, aumDollars: 100_000_000, primaryContact: { name: 'X', email: 'x@x.com', mobile: '+1 212 000 0001' } }), extendedCsms);
    const { csm: a2 } = router.assignCsm(makeDeal({ invoiceAmount: 5_000, aumDollars: 100_000_000, primaryContact: { name: 'X', email: 'x@x.com', mobile: '+1 212 000 0002' } }), extendedCsms);
    const { csm: a3 } = router.assignCsm(makeDeal({ invoiceAmount: 5_000, aumDollars: 100_000_000, primaryContact: { name: 'X', email: 'x@x.com', mobile: '+1 212 000 0003' } }), extendedCsms);
    expect(a1.name).toBe(a3.name);
  ...
  test('strictly regional: Global CSMs are NOT assigned to Americas contacts', () => {
    ...
    const { csm } = router.assignCsm(DEALS[1], CSMS);
    expect(csm.region).toBe('Americas');
    expect(csm.name).not.toBe('Hannah Petrov');
  });
